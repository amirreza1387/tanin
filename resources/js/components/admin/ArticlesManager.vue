...
const query = ref(''); const status = ref('all'); const page = ref(1); const pageSize = 20;
const pagination = ref({ current_page: 1, last_page: 1, total: 0, from: null, to: null });
const form = reactive({ id: '', title: '', lead: '', body: '', category_ids: [], featured_media_id: '', tag_ids: [], meta_title: '', meta_description: '' });
...
const pages = computed(() => {
  ...
  const start = Math.max(1, Math.min(page.value - 2, lastPage - 4));
  const end = Math.min(lastPage, start + 4);
  return Array.from({ length: end - start + 1 }, (_, index) => start + index);
});
const announce = (text) => { feedback.value = text; notify(text); window.setTimeout(() => { feedback.value = ''; }, 3500); };
const load = async () => { loading.value = true; error.value = ''; try { const [articleResult, categoryResult, tagResult, mediaResult] = await Promise.all([api.get('/management/articles', { params: { per_page: pageSize, page: page.value, q: query.value.trim() || undefined, status: status.value === 'all' ? undefined : status.value } }), api.get('/categories'), api.get('/tags'), api.get('/media?per_page=100')]); articles.value = articleResult.data.data || []; pagination.value = articleResult.data.meta || pagination.value; if (page.value > pagination.value.last_page) { page.value = pagination.value.last_page; return await load(); } categories.value = categoryResult.data.data || []; tags.value = tagResult.data.data || []; media.value = mediaResult.data.data || []; } catch (e) { error.value = firstError(e) || 'دریافت خبرها ممکن نشد.'; } finally { loading.value = false; } };
const applyFilters = () => { page.value = 1; load(); };
const changePage = (nextPage) => { if (nextPage === page.value || nextPage < 1 || nextPage > pageCount.value) return; page.value = nextPage; load(); };
const openNew = () => { Object.assign(form, { id: '', title: '', lead: '', body: '', category_ids: [], featured_media_id: '', tag_ids: [], meta_title: '', meta_description: '' }); editing.value = null; dirty.value = false; editorOpen.value = true; };
const openEdit = (article) => { Object.assign(form, { id: article.id, title: article.title, lead: article.lead || '', body: article.body || '', category_ids: article.categories?.map((category) => category.id) || (article.category ? [article.category.id] : []), featured_media_id: article.featured_media?.id || '', tag_ids: article.tags?.map((tag) => tag.id) || [], meta_title: article.seo?.meta_title || '', meta_description: article.seo?.meta_description || '' }); editing.value = article; dirty.value = false; editorOpen.value = true; };
...
const action = async (article, name) => { const snapshot = [...articles.value]; try { if (name === 'delete') articles.value = articles.value.filter((item) => item.id !== article.id); else article.status = name === 'publish' ? 'published' : 'draft'; if (name === 'delete') await api.delete(`/articles/${article.id}`); else await api.post(`/articles/${article.id}/${name}`); announce(name === 'publish' ? 'خبر منتشر شد.' : name === 'revert' ? 'خبر به پیش‌نویس بازگشت.' : 'خبر حذف شد.'); await load(); } catch (e) { articles.value = snapshot; error.value = firstError(e); notify(error.value, 'error'); } };
...
const uploadMedia = async (file) => { if (!file?.type.startsWith('image/')) throw new Error('برای تصویر فقط فایل تصویری انتخاب کنید.'); uploading.value = true; try { const data = new FormData(); data.append('file', file); const result = await api.post('/media', data, { headers: { 'Content-Type': 'multipart/form-data' } }); media.value.unshift(result.data.data); return result.data.data; } finally { uploading.value = false; } };
const upload = async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const item = await uploadMedia(file); form.featured_media_id = item.id; announce('تصویر شاخص آپلود و انتخاب شد.'); } catch (e) { error.value = firstError(e) || e.message; } finally { event.target.value = ''; } };
const pasteImage = async ({ file, insert }) => { try { const item = await uploadMedia(file); await insert(item.url); announce('تصویر میان متن خبر قرار گرفت.'); } catch (e) { error.value = firstError(e) || e.message; notify(error.value, 'error'); } };
onMounted(load);
</script>
...
<template>
  <section class="space-y-4">
    ...
    <ArticleEditor v-if="editorOpen" :article="form" :editing="Boolean(editing)" :categories="categories" :tags="tags" :media="media" :saving="saving" :uploading="uploading" @save="save" @upload="upload" @paste-image="pasteImage" @dirty-change="dirty = $event" @cancel="closeEditor" />
    ...
    <AsyncState :loading="loading" skeleton="table" :error="error" :empty="!loading && !error && !articles.length" empty-text="هنوز خبری برای مدیریت وجود ندارد." @retry="load">
      <Card class="grid gap-3 sm:grid-cols-[1fr_190px_auto]"><Input id="article-search" v-model="query" placeholder="جست‌وجوی عنوان خبر" aria-label="جست‌وجوی خبر" @keyup.enter="applyFilters" /><Select id="article-status" v-model="status" aria-label="فیلتر وضعیت" :options="[{ value: 'all', label: 'همه وضعیت‌ها' }, { value: 'draft', label: 'پیش‌نویس' }, { value: 'published', label: 'منتشرشده' }, { value: 'scheduled', label: 'زمان‌بندی‌شده' }]" @update:modelValue="applyFilters" /><div class="flex items-center justify-between gap-3"><span class="text-xs text-muted">{{ pagination.total }} نتیجه</span><Button variant="secondary" size="sm" @click="applyFilters">جست‌وجو</Button></div></Card>
      ...
      <nav v-if="pageCount > 1" class="ds-pagination mt-4" aria-label="صفحه‌بندی خبرها"><Button variant="secondary" size="sm" :disabled="page === 1" @click="changePage(page - 1)">قبلی</Button><Button v-for="item in pages" :key="item" variant="secondary" size="sm" :aria-current="page === item ? 'page' : undefined" @click="changePage(item)">{{ item }}</Button><Button variant="secondary" size="sm" :disabled="page === pageCount" @click="changePage(page + 1)">بعدی</Button></nav>
    </AsyncState>
  </section>
</template>
