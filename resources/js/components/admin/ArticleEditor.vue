...
const emit = defineEmits(['save', 'cancel', 'upload', 'paste-image', 'dirty-change']);
...
const bodyInput = ref(null); const pasteError = ref('');
...
const pasteImage = (event) => {
    ...
    if (props.uploading) { pasteError.value = 'آپلود تصویر قبلی هنوز تمام نشده است.'; return; }
    pasteError.value = '';
    ...
    emit('paste-image', {
        ...
        insert: async (url) => {
            const markup = `\n\n![تصویر خبر](${url})\n\n`;
            draft.body = `${draft.body.slice(0, start)}${markup}${draft.body.slice(end)}`;
            await nextTick();
            bodyInput.value?.focus();
            bodyInput.value?.setSelectionRange(start + markup.length, start + markup.length);
        },
    });
};
let autosaveTimer;
...
<template>
    <Card class="border border-divider" :padded="false">
        ...
        <form class="grid gap-5 p-5" @submit.prevent="step === 3 ? submit() : next($event)">
            <div v-if="step === 1" class="grid gap-4"><Input v-model="draft.title" label="عنوان خبر" id="article-title" required :error="errors.title" placeholder="عنوان خبر را وارد کنید" /><div><label for="article-lead" class="ds-label">خلاصه خبر</label><textarea id="article-lead" v-model="draft.lead" class="ds-control min-h-24 w-full" placeholder="خلاصه کوتاه و قابل فهم"></textarea></div><div><label for="article-body" class="ds-label">متن کامل خبر <span class="text-red-600">*</span></label><textarea id="article-body" ref="bodyInput" v-model="draft.body" class="ds-control min-h-64 w-full" :class="errors.body ? 'border-red-500' : ''" placeholder="متن کامل خبر" @paste="pasteImage"></textarea><p class="mt-1 text-xs text-muted">برای قرار دادن عکس میان متن، تصویر را در محل دلخواه Paste کنید.</p><p v-if="pasteError" class="mt-1 text-xs text-red-700" role="alert">{{ pasteError }}</p><p v-if="errors.body" class="mt-1 text-xs text-red-700" role="alert">{{ errors.body }}</p></div></div>
            <div v-else-if="step === 2" class="grid gap-4"><fieldset><legend class="ds-label">دسته‌بندی‌ها</legend><div class="flex flex-wrap gap-2"><label v-for="category in categories" :key="category.id" class="rounded-full bg-cream px-3 py-2 text-xs"><input v-model="draft.category_ids" type="checkbox" :value="category.id" class="ml-1">{{ category.name }}</label></div></fieldset><div><label class="ds-label">تصویر شاخص</label><MediaPicker v-model="draft.featured_media_id" :media="media" :uploading="uploading" @upload="emit('upload', $event)" /><div v-if="selectedMedia" class="mt-3 h-36 overflow-hidden rounded-lg bg-cream"><img :src="selectedMedia.url" alt="پیش‌نمایش تصویر شاخص" class="h-full w-full object-contain"></div></div><fieldset><legend class="ds-label">برچسب‌ها</legend><div class="flex flex-wrap gap-2"><label v-for="tag in tags" :key="tag.id" class="rounded-full bg-cream px-3 py-2 text-xs"><input v-model="draft.tag_ids" type="checkbox" :value="tag.id" class="ml-1">{{ tag.name }}</label></div></fieldset></div>
            <div v-else class="rounded-xl bg-cream p-5"><h3 class="text-xl font-black">{{ draft.title || 'بدون عنوان' }}</h3><p v-if="draft.lead" class="mt-3 font-bold leading-8">{{ draft.lead }}</p><ArticleContent v-if="draft.body" :content="draft.body" class="mt-4 text-sm leading-8" /><div v-else class="mt-4 text-sm text-muted">متن خبر هنوز وارد نشده است.</div><div class="mt-5 border-t border-divider pt-4 text-xs text-muted">{{ categories.filter((item) => draft.category_ids?.includes(item.id)).map((item) => item.name).join('، ') || 'بدون دسته‌بندی' }}</div></div>
