...
class EloquentArticleRepository implements ArticleRepositoryInterface
{
    ...
    {
        $query = Article::query()
            ->with(['author', 'category', 'categories', 'tags', 'featuredMedia'])
    ...
    public function findPublishedBySlug(string $slug): Article
    {
        return Article::query()
            ->with(['author', 'category', 'categories', 'tags', 'featuredMedia', 'comments' => fn ($query) => $query
    ...
    {
        return Article::query()
            ->with(['category', 'categories', 'featuredMedia'])
            ->where('status', 'published')
    ...
    {
        $query->when($filters['category_id'] ?? null, fn ($builder, $categoryId) => $builder->where(fn ($articles) => $articles
            ->where('category_id', $categoryId)
            ->orWhereHas('categories', fn ($categories) => $categories->whereKey($categoryId))));
        $query->when($filters['category_slug'] ?? null, fn ($builder, $slug) => $builder->where(fn ($articles) => $articles
            ->whereHas('category', fn ($category) => $category->where('slug', $slug))
            ->orWhereHas('categories', fn ($categories) => $categories->where('slug', $slug))));
